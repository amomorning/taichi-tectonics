...
ti.init(arch=ti.gpu)  # set arch=ti.gpu if you have cuda backend
...
woodTex = load_texture("./imgs/wood.jpg")
skyboxTex = load_texture("./imgs/skybox.jpg")
...
def render(time: ti.f32):
    ...
    camera_location = 5 * vec3(ti.cos(mx) * ti.cos(my),
                               ti.sin(my),
                               ti.sin(mx) * ti.cos(my))
    lookat = vec3(0, 0, 0)
